...
include $(ROOTDIR)/module/network.mk
...
$(NETWORK_INFO):
	...
	@curl -s -f -m 3 $(NETWORK_URL)/$(notdir $@) --output $@ || \
		([ -e $@ ] && echo "Using old satellite $(notdir $@)") || (echo "\e[33mError: Satellite $(notdir $@) not found\e[0m"; exit 1)

.PHONY: $(LATEST_NETWORK_INFO)
$(LATEST_NETWORK_INFO):
	@(curl -s -f $(NETWORK_URL)/$(SATELLITES) -o $@.bin && echo "Latest network information has been downloaded to $@.bin.") || \
